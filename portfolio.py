# -*- coding: utf-8 -*-
"""
portfolio.py — พอร์ตหุ้นที่ถืออยู่ (5 บัญชี) + ปันผล/XD + แผนแก้หุ้นติด
...
COLUMNS = ["บัญชี", "หุ้น", "จำนวนหุ้น", "ราคาต้นทุน", "วันที่ซื้อ",
           "ปันผลต่อหุ้น", "วันที่ XD", "หมายเหตุ"]

NUM_COLS = ["จำนวนหุ้น", "ราคาต้นทุน", "ปันผลต่อหุ้น"]
DATE_COLS = ["วันที่ซื้อ", "วันที่ XD"]
...
def enrich(df: pd.DataFrame, last_price: dict, engine: dict | None = None,
    ...
    for _, r in df.iterrows():
        ...
        rows.append({
            ...
            "วันที่ XD": r["วันที่ XD"], "หมายเหตุ": r["หมายเหตุ"],
        })
...
DISCLAIMER = (
    ...
    "ระบบไม่มีข้อมูลงบการเงิน ประวัติปันผล หรือวัน XD ย้อนหลัง — ส่วนนั้นคุณกรอกเอง · "
